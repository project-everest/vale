...
=
  ...
  | hd :: tl ->
      ...
      let foo (j:int) :   // TODO: Why do I have to add "j < length cipher" below!?
	Lemma ((0 <= j /\ j < length plain /\ j < length cipher) ==> (nth_in_bounds cipher j == quad32_xor (nth_in_bounds plain j) (aes_encrypt alg key (inc32 icb (i + j)) )))
	= 
	if j <= 0 then ()
	  ...
	  gctr_indexed icb tl alg key (i+1);
	  assert(nth_in_bounds r_cipher (j-1) == quad32_xor (nth_in_bounds tl (j-1)) (aes_encrypt alg key (inc32 icb (i + 1 + j - 1)) )) // OBSERVE
	  //assert(nth_in_bounds plain j == nth_in_bounds tl (j-1))
	) else ()	
      in
      FStar.Classical.forall_intro foo
...
=
  ...
  assert(forall j . 0 <= j /\ j < length plain ==> nth_in_bounds cipher j == nth_in_bounds c j);
  ()
