...
val addrs_set: (ptrs:list (B.buffer UInt8.t)) -> (addrs:addr_map) -> GTot (s:Set.set int{
  forall x. not (Set.mem x s) <==> 
    (forall (b:B.buffer UInt8.t{List.memP b ptrs}). x < addrs b \/ x >= addrs b + B.length b)})

private let add_buffer_domain (b:B.buffer UInt8.t) (addrs:addr_map) (accu:Set.set int) : GTot (s:Set.set int{
...
private let rec addrs_set_aux (ptrs:list (B.buffer UInt8.t))
			      (ps2: list (B.buffer UInt8.t))
			      (ps:list (B.buffer UInt8.t){forall b. List.memP b ptrs <==> List.memP b ps \/ List.memP b ps2})
			      (addrs:addr_map)
			      (accu:Set.set int{forall (x:int). not (Set.mem x accu) <==>
			        (forall (b:B.buffer UInt8.t{List.memP b ps}). x < addrs b \/ x >= addrs b + B.length b)}) :
  ...
  | a::q -> 
    ...
    let aux (x:int) : Lemma
      ...
      (forall (b:B.buffer UInt8.t{List.memP b (a::ps)}). 
...
val addrs_set_lemma: (ptrs1:list (B.buffer UInt8.t)) -> (ptrs2:list (B.buffer UInt8.t)) ->
...
val addrs_set_concat: (ptrs:list (B.buffer UInt8.t)) -> (a:B.buffer UInt8.t) ->
  (addrs:addr_map) -> Lemma
...
val addrs_set_mem: (ptrs:list (B.buffer UInt8.t)) -> (a:B.buffer UInt8.t) ->
...
let write_buffer_vale (a:B.buffer UInt8.t) (heap:heap) (mem:HS.mem) (addrs:addr_map) =
  let length = B.length a in
  let contents = B.as_seq mem a in
  let addr = addrs a in
  write_vale_mem contents length addr 0 heap

let domain_write_buffer (a:B.buffer UInt8.t) (heap:heap) (mem:HS.mem) (addrs:addr_map) : Lemma
...
private
let rec frame_down_mem_aux (ptrs:list (B.buffer UInt8.t){list_disjoint_or_eq ptrs})
  (addrs:addr_map)
  (mem:HS.mem)
  (ps:list (B.buffer UInt8.t))
  (accu:list (B.buffer UInt8.t){forall p. List.memP p ptrs <==> List.memP p ps \/ List.memP p accu})
