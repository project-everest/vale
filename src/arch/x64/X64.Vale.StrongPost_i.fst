...
let lemma_valid_maddr_eq (r:range) (addr:maddr) (s:state) :
  Lemma (valid_maddr_norm addr s <==> valid_maddr r addr s) =
    assert_from_norm' (valid_maddr_norm addr s <==> valid_maddr r addr s)

let lemma_valid_op_eq (r:range) (op:operand) (s:state) :
  Lemma (valid_operand_norm op s <==> valid_operand r op s) =
   let _ =  match op with
	    | OConst n -> ()
	    | OReg r -> ()
	    | OMem m -> lemma_valid_maddr_eq r m s
   in
     assert_from_norm' (valid_operand_norm op s <==> valid_operand r op s)
...
let lemma_weak_pre_ins (i:ins) (inss:list ins) 
  ...
  let r_info = range_of i in
  match i with
  | Mov64 (OReg dst) src ->
    let _ = lemma_valid_op_eq r_info src s0 in
  ...
  | Load64 (OReg dst) (OReg src) offset ->
    let _ = lemma_valid_maddr_eq r_info (MReg src offset) s0 in
      if dst <> Rsp && valid_maddr_norm (MReg src offset) s0 then
  ...
  | Mul64Wrap src ->
    let _ = lemma_valid_op_eq r_info src s0 in
      if valid_operand_norm src s0 then
  	  ...
  	  None
  | IMul64 (OReg dst) src ->
      lemma_valid_op_eq r_info src s0;
