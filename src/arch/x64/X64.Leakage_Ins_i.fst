...
val check_if_ins_consumes_fixed_time: (ins:tainted_ins{not (is_xmm_ins ins)}) -> (fuel:nat) -> (ts:taintState) -> (res:(bool*taintState){ins_consumes_fixed_time ins fuel ts res})
...
let check_if_ins_consumes_fixed_time ins fuel ts =
  ...
  Classical.forall_intro_2 (lemma_operand_obs_list ts srcs);
...
val lemma_ins_same_public: (ts:taintState) -> (fuel:nat) -> (ins:tainted_ins{not (is_xmm_ins ins)}) -> (s1:traceState) -> (s2:traceState) -> Lemma
(let b, ts' = check_if_ins_consumes_fixed_time ins fuel ts in
  (b2t b ==> isExplicitLeakageFreeGivenStates (Ins ins) fuel ts ts' s1 s2))
...
val lemma_ins_leakage_free: (ts:taintState) -> (fuel:nat) -> (ins:tainted_ins{not (is_xmm_ins ins)}) -> Lemma
 (let b, ts' = check_if_ins_consumes_fixed_time ins fuel ts in
