...
let rec wp_code (inss : list ins) (post: state -> Type0) (s0:state) : Type0 =
  ...
  | hd :: inss ->
	...
	valid_operand (range_of hd) src s0 /\
	valid_maddr (range_of hd) (MReg dst offset) s0 /\
	(forall x.
	...
	let a = s0.regs dst `op_Multiply` eval_operand_norm src s0 in
	  (valid_operand (range_of hd) src s0) /\
	  labeled (range_of hd) "Multiplication result overflowed" (a < nat64_max) /\
...
val va_lemma_weakest_pre_norm (inss:list ins) (s0:state) (sN:state) : PURE unit
  (fun (post:(unit -> Type)) ->
	...
	Prims.norm [delta_only wp_code_delta; iota; zeta; primops]
...
// #reset-options "--debug X64.Vale.StrongPost_i --debug_level print_normalized_terms"
...
//     let _ = va_lemma_weakest_pre_norm (test_code ()) s0 sN in
