...
let wp_code_delta = [
  ...
  "X64.Vale.StrongPost_i.augment";
  "X64.Vale.StrongPost_i.va_ins_range";
  "Test.FastBlock.va_ins_1_Copy64"
  ]

let va_ins_range_aux (filename:string) (line:int) (col:int) (i:ins) =
  Prims.set_range_of i (mk_range filename line col line col)  
let va_ins_range (filename:string) (line:int) (col:int) (i:ins) =
    (Prims.set_range_of (normalize_term i) (mk_range filename line col line col))
...
let valid_maddr (r:range) (addr:maddr) (s:state) : Type0 =
  let msg = normalize_term (
      strcat (strcat ("Memory address ") (string_of_maddr addr))
	     (" is invalid")) in
 labeled r msg (Map.contains s.mem (eval_maddr addr s))
...
let rec wp_code (inss : list ins) (post: state -> Type0) (s0:state) : Type0 =
  ...
  | hd :: inss ->
    begin
      ...
      | (Mov64 (OReg Rsp) _) -> False
      | (Mov64 (OReg dst) src) ->
	valid_operand (range_of hd) src s0 /\
	(forall x. x == eval_operand_norm src s0 ==>
              wp_code inss post (update_reg dst x s0))
      | (Load64 (OReg Rsp) _ _) -> False
      | (Load64 (OReg dst) (OReg src) offset) ->
	  (set_range_of (b2t (Map.contains s0.mem (s0.regs src + offset))) (range_of hd)) /\
	// valid_maddr (range_of hd) (MReg src offset) s0 /\
	// valid_maddr_norm (MConst (s0.regs src + offset)) s0 /\
	(forall x. x == Map.sel s0.mem (s0.regs src + offset) ==>
              wp_code inss post (update_reg dst x s0))
      | (Store64 (OReg dst) src offset) ->
...
let test_code () =
[(va_ins_range "bla" 1 0 (va_fast_ins_Load64 (OReg Rax) (OReg Rsi) 0));
 (va_ins_range "blu" 1 0 (va_fast_ins_Load64 (OReg Rbx) (OReg Rcx) 0))]
 
let test_lemma (s0:state) (sN:state) =
    assume (s0.ok);
    // assume (Map.contains s0.mem (s0.regs Rsi));
    assume (Map.contains s0.mem (s0.regs Rcx));
    // assume (Some sN == va_eval_code (va_Block (normalize_term (inss_to_codes (test_code ())))) s0);
    let _ = va_lemma_weakest_pre_norm ([(va_ins_range "bla" 1 0 (va_fast_ins_Load64 (OReg Rax) (OReg Rsi) 0));
 (va_ins_range "blu" 1 0 (va_fast_ins_Load64 (OReg Rbx) (OReg Rcx) 0))]) s0 sN in
    //this assertion is what F* uses to implicitly instantiate
    //the post-condition predicate in lemma_weakest_pre_norm
    assert (state_eq sN (update_reg Rbx (sN.regs Rbx)
                        (update_reg Rax (sN.regs Rax) s0)))
