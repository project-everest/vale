...
// let rec weak_pre (inss: list ins) (s0:state) (sN:state) : Type0 =
//   match inss with
//   | [] -> 
//     s0.ok /\ 
//     s0.flags == sN.flags /\ 
...
//      sN.regs dst == eval_operand_norm src s0 /\
//      weak_pre inss s0 (update_reg dst (s0.regs dst) sN) 
//   | (Load64 (OReg Rsp) _ _)::_ -> True
//   | (Load64 (OReg dst) (OReg src) offset)::inss ->
//     (mem_contains s0.mem (s0.regs src + offset)) /\
//      sN.regs dst == mem_sel s0.mem (s0.regs src + offset) /\
//      weak_pre inss s0 (update_reg dst (s0.regs dst) sN)
//   | (Store64 (OReg dst) src offset)::inss ->
//       (valid_operand_norm src s0) /\
//        mem_contains s0.mem (s0.regs dst + offset) /\
//       forall x.
// 	(not (x == mem_upd sN.mem (s0.regs dst + offset) (mem_sel s0.mem (s0.regs src + offset))) \/
// 	(mem_sel sN.mem (s0.regs dst + offset) == mem_sel s0.mem (s0.regs src + offset) /\
// 	 weak_pre inss s0 ({sN with mem = x}))
//   | (Add64Wrap (OReg Rsp) _)::_ -> True
//   | (Add64Wrap (OReg dst) src)::inss ->
//       (valid_operand_norm src s0) /\
//       (forall a x f. 
// 	(not (a == s0.regs dst + eval_operand_norm src s0 /\
// 	      x == if a < nat64_max then a else a - nat64_max /\
// 	      cf f == (a >= nat64_max))) \/
// 	(weak_pre inss s0 ({update_reg dst (s0.regs dst) sN with flags = s0.flags})))
//   | (Adc64Wrap (OReg Rsp) _)::_ -> True
//   | (Adc64Wrap (OReg dst) src)::inss ->
//       (valid_operand_norm src s0) /\
//       (forall a x f.
//         (not (a == s0.regs dst + eval_operand_norm src s0 + (if cf s0.flags then 1 else 0) /\
//              x == (if a < nat64_max then a else a - nat64_max) /\
//              cf f == (a >= nat64_max))) \/
//         weak_pre inss s0 ({update_reg dst (s0.regs dst) sN with flags = s0.flags})
//   | (Mul64Wrap src)::inss ->
...
//         (not (nat64_max `op_Multiply` rdx + rax == s0.regs Rax `op_Multiply` eval_operand_norm src s0))
//         strong_post inss (update_reg Rdx rdx (update_reg Rax rax ({s0 with flags = f}))) sN)
//   | (IMul64 (OReg Rsp) _)::_ -> True
//   | (IMul64 (OReg dst) src)::inss ->
//       let a = s0.regs dst `op_Multiply` eval_operand_norm src s0 in
...
[@"opaque_to_smt"]
let rec wp_code (inss : list ins) (post: state -> Type0) : Tot (state -> Type0) =
  fun s0 ->
    match inss with
    | [] ->
      post s0 /\
      s0.ok
    | (Mov64 (OReg Rsp) _) :: inss -> wp_code inss post s0
    | (Mov64 (OReg dst) src) :: inss ->
      let post' = wp_code inss post in
      (valid_operand_norm src s0) /\
      post' (update_reg dst (eval_operand_norm src s0) s0)
    | (Load64 (OReg Rsp) _ _) :: inss -> wp_code inss post s0
    | (Load64 (OReg dst) (OReg src) offset) :: inss ->
      let post' = wp_code inss post in
      (mem_contains s0.mem (s0.regs src + offset)) /\
      post' (update_reg dst (mem_sel s0.mem (s0.regs src + offset)) s0)
    | (Store64 (OReg dst) src offset) :: inss ->
      let post' = wp_code inss post in
      (valid_operand_norm src s0) /\
      (mem_contains s0.mem (s0.regs dst + offset)) /\
...
// #reset-options "--max_fuel 50 --z3rlimit_factor 2"
// [@"opaque_to_smt"]
// let rec wp_code (inss : list ins) (post: state -> Type0) : state -> Type0  =
//   fun s0 ->
//     match inss with
//     | [] ->
//       post s0 /\
//       s0.ok
//     | (Mov64 (OReg Rsp) _) :: inss -> wp_code inss post
//     | (Mov64 (OReg dst) src) :: inss ->
//       let post' = wp_code inss post in
//       (valid_operand_norm src s0) /\
//       (forall vdst'. (~ (vdst' == eval_operand_norm src s0)) \/
// 	      post' (update_reg dst vdst' s0))
//     | (Load64 (OReg Rsp) _ _) :: inss -> wp_code inss post
//     | (Load64 (OReg dst) (OReg src) offset) :: inss ->
//       let post' = wp_code inss post in
//       (mem_contains s0.mem (s0.regs src + offset)) /\
//       (forall vdst'. (~ (vdst' == mem_sel s0.mem (s0.regs src + offset))) \/
// 	      post' (update_reg dst vdst' s0))
//     | (Store64 (OReg dst) src offset) :: inss ->
//       let post' = wp_code inss post in
//       (valid_operand_norm src s0) /\
//       (mem_contains s0.mem (s0.regs dst + offset)) /\
//       (forall vdst'. (~ (vdst' == eval_operand_norm src s0)) \/
// 	     post' (update_mem (s0.regs dst + offset) vdst' s0))
//     | (Add64Wrap (OReg Rsp) _) :: inss -> wp_code inss post
//     | (Add64Wrap (OReg dst) src) :: inss ->
//       let post' = wp_code inss post in
//       (valid_operand_norm src s0) /\
//       (forall a x f. 
...
// 	      cf f == (a >= nat64_max))) \/
// 	post' ({update_reg dst x s0 with flags = f}))
//     | (Adc64Wrap (OReg Rsp) _) :: inss -> wp_code inss post
//     | (Adc64Wrap (OReg dst) src) :: inss ->
//       let post' = wp_code inss post in
//       (valid_operand_norm src s0) /\
//       (forall a x f.
...
val va_lemma_wp_norm : inss:list ins -> post:(state -> Type0) -> s0:state -> Lemma
  (requires wp_code inss post s0)
  (ensures
    (forall sN.
     Some sN == va_eval_code (va_Block (normalize_term (inss_to_codes inss))) s0 ==>
     post sN))
    ...
    //   ok0 == s0.ok /\ regs0 == s0.regs /\ flags0 == s0.flags /\ mem0 == s0.mem /\
    //   okN == sN.ok /\ regsN == sN.regs /\ flagsN == sN.flags /\ memN == sN.mem /\
    //   normalize (p_code inss
    //     (fun s -> s == {ok = okN; regs = regsN; flags = flagsN; mem = memN})
    //     (fun s -> s == {ok = ok0; regs = regs0; flags = flags0; mem = mem0})))
