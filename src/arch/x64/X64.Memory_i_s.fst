...
let buffer_readable #t h b = B.live h b
...
let valid_state s = 
  // Shouldn't be needed but logic annotation issue without it
  let _ = () in
  I.list_disjoint_or_eq #UInt64.t s.ptrs /\
  I.list_live s.mem s.ptrs /\
  s.state.S.mem == I.down_mem64 s.mem s.addrs s.ptrs
...
let rec valid_mem_aux addr ps (s:state) : GTot (b:bool{
  (not b) <==> (forall (x:buffer64). (List.memP x ps ==> not (addr_in_ptr addr x s) ))}) 
  = match ps with
    | [] -> false
    | a::q -> if (addr_in_ptr addr a s) then true else valid_mem_aux addr q s

let valid_mem64 ptr s = valid_mem_aux ptr s.ptrs s

// TODO: Probably a similar approach baser on buffer_read?
let load_mem64 ptr s = S.get_heap_val64 ptr s.state.S.mem

let rec store_mem_aux addr ps (v:nat64) (s:state) (h0:HS.mem{I.list_live h0 ps}) : 
  GTot (h1:HS.mem{forall (b:buffer64). B.live h0 b <==> B.live h1 b }) =
  match ps with
  | [] -> h0
  | a::q ->
    let base = buffer_addr a s in
    let n = B.length a in
    if addr_in_ptr addr a s then
    begin
      buffer_write a (get_addr_in_ptr n base addr 0) v h0
    end
    else store_mem_aux addr q v s h0
...
let rec written_buffer_down_aux1 (b:buffer64) (i:nat{i < B.length b}) (v:nat64)
      ...
      (h0:HS.mem{I.list_live h0 ps}) (s:state)
      (base:nat{base == buffer_addr b s})
      (k:nat) (h1:HS.mem{h1 == buffer_write b i v h0}) 
      (mem1:S.heap{I.correct_down64 h0 s.addrs ps mem1}) 
      (mem2:S.heap{
      (I.correct_down64 h1 s.addrs ps mem2) /\
    ...
    if k >= i then ()
      ...
      written_buffer_down_aux1 b i v ps h0 s base (k+1) h1 mem1 mem2
    end
...
let rec written_buffer_down_aux2 (b:buffer64) (i:nat{i < B.length b}) (v:nat64)
    ...
    if k >= n then ()
      ...
      assert (Seq.index (B.as_seq h0 b) k == Seq.index (B.as_seq h1 b) k);
      Bytes_Semantics_i.same_mem_get_heap_val (base + 8 `op_Multiply` k) mem1 mem2;
      written_buffer_down_aux2 b i v ps h0 s base n (k+1) h1 mem1 mem2
    end
    
let written_buffer_down (b:buffer64) (i:nat{i < B.length b}) (v:nat64)
  (ps: list buffer64{I.list_disjoint_or_eq ps /\ List.memP b ps}) (h0:HS.mem{I.list_live h0 ps}) (s:state) :
  Lemma (
    let mem1 = I.down_mem64 h0 s.addrs ps in
    let h1 = buffer_write b i v h0 in
    let mem2 = I.down_mem64 h1 s.addrs ps in
    let base = buffer_addr b s in
    let n = B.length b in
    ...
    let base = buffer_addr b s in
    let n = B.length b in
    written_buffer_down_aux1 b i v ps h0 s base 0 h1 mem1 mem2;
    written_buffer_down_aux2 b i v ps h0 s base n (i+1) h1 mem1 mem2
...
let rec unwritten_buffer_down_aux (b:buffer64) (i:nat{i < B.length b}) (v:nat64)
  (ps: list buffer64{I.list_disjoint_or_eq ps /\ List.memP b ps})
  (h0:HS.mem{I.list_live h0 ps}) 
  (s:state) (a:buffer64{a =!= b /\ List.memP a ps})  : 
  Lemma (let base = buffer_addr a s in
    let mem1 = I.down_mem64 h0 s.addrs ps in
    let h1 = buffer_write b i v h0 in
    let mem2 = I.down_mem64 h1 s.addrs ps in
...
let unwritten_buffer_down (b:buffer64) (i:nat{i < B.length b}) (v:nat64)
  (ps: list buffer64{I.list_disjoint_or_eq ps /\ List.memP b ps}) (h0:HS.mem{I.list_live h0 ps}) (s:state) : Lemma (
    let mem1 = I.down_mem64 h0 s.addrs ps in
    let h1 = buffer_write b i v h0 in
    let mem2 = I.down_mem64 h1 s.addrs ps in
    forall  (a:buffer64{List.memP a ps /\ a =!= b}) j.
    let base = buffer_addr a s in j >= base /\ j < base + B.length a `op_Multiply` 8 ==> mem1.[j] == mem2.[j]) =
    let mem1 = I.down_mem64 h0 s.addrs ps in
    let h1 = buffer_write b i v h0 in
    let mem2 = I.down_mem64 h1 s.addrs ps in   
    let fintro (a:buffer64{List.memP a ps /\ a =!= b}) 
      : Lemma 
      (forall j. let base = buffer_addr a s in 
      j >= base /\ j < base + (B.length a) `op_Multiply` 8 ==> 
	mem1.[j] == mem2.[j]) =
      let base = buffer_addr a s in
      unwritten_buffer_down_aux b i v ps h0 s a
    in
...
let store_buffer_down_mem (b:buffer64) (i:nat{i < B.length b}) (v:nat64)
  ...
  Lemma (
    ...
    let base = buffer_addr b s in
    let n = B.length b in
    I.same_unspecified_down h0 h1 s.addrs ps;
    unwritten_buffer_down b i v ps h0 s;
    assert (forall j. (exists (a:buffer64{List.memP a ps /\ a =!= b}) . let base = buffer_addr a s in
      j >= base /\ j < base + B.length a `op_Multiply` 8) ==> mem1.[j] == mem2.[j]);

    written_buffer_down b i v ps h0 s;
    ()

let rec get_addr_ptr (ptr:int) (s:state) (ps:list buffer64{valid_mem_aux ptr ps s}) : 
  GTot (b:buffer64{List.memP b ps /\ addr_in_ptr ptr b s}) =
  match ps with
  // The list cannot be empty because of the mem predicate
  | a::q -> if addr_in_ptr ptr a s then a else get_addr_ptr ptr s q

let store_buffer_aux_down_mem (ptr:int) (v:nat64) (s:state{valid_mem64 ptr s}) : Lemma (
  let mem1 = I.down_mem64 s.mem s.addrs s.ptrs in
  let h1 = store_mem_aux ptr s.ptrs v s s.mem in
  let mem2 = I.down_mem64 h1 s.addrs s.ptrs in
  forall j. j < ptr \/ j >= ptr + 8 ==> mem1.[j] == mem2.[j]) =
  let h1 = store_mem_aux ptr s.ptrs v s s.mem in
  let b = get_addr_ptr ptr s s.ptrs in
  let i = get_addr_in_ptr (B.length b) (buffer_addr b s) ptr 0 in
  let rec aux (ps:list buffer64{I.list_disjoint_or_eq ps /\ valid_mem_aux ptr ps s}) (h:HS.mem{I.list_live h ps}) :
  Lemma (let b = get_addr_ptr ptr s ps in
    let i = get_addr_in_ptr (B.length b) (buffer_addr b s) ptr 0 in
    store_mem_aux ptr ps v s h == buffer_write b i v h) =
    match ps with
    | [] -> ()
    | a::q ->
      if addr_in_ptr ptr a s then () else aux q h    
  in aux s.ptrs s.mem;
  store_buffer_down_mem b i v s.ptrs s.mem s

let store_buffer_aux_down_mem2 (ptr:int) (v:nat64) (s:state{valid_mem64 ptr s}) : Lemma (
  let h1 = store_mem_aux ptr s.ptrs v s s.mem in
  let mem2 = I.down_mem64 h1 s.addrs s.ptrs in
...
let store_mem64 i v s =
  if not (valid_mem64 i s) then s
  else
  let s' = S.update_mem i v s.state in
  let h1 = store_mem_aux i s.ptrs v s s.mem in
  ...
  let mem2 = I.down_mem64 s'.mem s.addrs s.ptrs in
