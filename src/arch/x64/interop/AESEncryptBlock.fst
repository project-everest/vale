...
open Vale_aes128_encrypt_block
...
(  let buffers = output_b::input_b::keys_b::[] in
  ...
  va_pre (va_code_aes128_encrypt_block ()) s0 output_b input_b (Ghost.reveal key) keys_b ))) =
  length_t_eq (TBase TUInt128) output_b;
  length_t_eq (TBase TUInt128) input_b;
  length_t_eq (TBase TUInt128) keys_b;
  ()
...
let implies_post (va_s0:va_state) (va_sM:va_state) (va_fM:va_fuel) (output_b:b8) (input_b:b8) (key:Ghost.erased (aes_key_LE AES_128)) (keys_b:b8)  : Lemma
  (requires pre_cond va_s0.mem.hs output_b input_b key keys_b /\
    va_post (va_code_aes128_encrypt_block ()) va_s0 va_sM va_fM output_b input_b (Ghost.reveal key) keys_b )
...
val ghost_aes128_encrypt_block: output_b:b8 -> input_b:b8 -> key:Ghost.erased (aes_key_LE AES_128) -> keys_b:b8 -> (h0:HS.mem{pre_cond h0 output_b input_b key keys_b }) -> GTot (h1:HS.mem{post_cond h0 h1 output_b input_b key keys_b })

let ghost_aes128_encrypt_block output_b input_b key keys_b h0 =
  let buffers = output_b::input_b::keys_b::[] in
  let (mem:mem) = {addrs = addrs; ptrs = buffers; hs = h0} in
  let addr_output_b = addrs output_b in
  let addr_input_b = addrs input_b in
  ...
  let s1, f1 = va_lemma_aes128_encrypt_block (va_code_aes128_encrypt_block ()) s0 output_b input_b (Ghost.reveal key) keys_b  in
  ...
  // Ensures that va_code_aes128_encrypt_block is actually Vale code, and that s1 is the result of executing this code
  assert (va_ensure_total (va_code_aes128_encrypt_block ()) s0 s1 f1);
  implies_post s0 s1 f1 output_b input_b key keys_b ;
  s1.mem.hs

let aes128_encrypt_block output_b input_b key keys_b  =
  let h0 = get() in
  st_put h0 (fun h -> pre_cond h output_b input_b key keys_b ) (ghost_aes128_encrypt_block output_b input_b key keys_b )
