...
(plain_b:buffer128) (mask_b:buffer128) (cipher_b:buffer128) (offset:nat64) (num_blocks:(nat64)) (key:(aes_key_LE AES_128)) (iv:(quad32))  =
  ((va_require_total va_b0 (va_code_gcm_load_xor_store_buffer_win ()) va_s0) /\
    (va_get_ok va_s0) /\ (locs_disjoint [(loc_buffer stack_b); (loc_buffer plain_b); (loc_buffer
    mask_b); (loc_buffer cipher_b)]) /\ (buffer_readable (va_get_mem va_s0) stack_b) /\
    (buffer_readable (va_get_mem va_s0) plain_b) /\ (buffer_readable (va_get_mem va_s0) mask_b) /\
    (buffer_readable (va_get_mem va_s0) cipher_b) /\ (buffer_length stack_b) >= 5 /\
    ...
    num_blocks /\ mask == (aes_encrypt_BE AES_128 key (inc32 iv offset)) /\ (gctr_partial AES_128
    offset plain cipher key iv)))
...
(plain_b:buffer128) (mask_b:buffer128) (cipher_b:buffer128) (offset:nat64) (num_blocks:(nat64)) (key:(aes_key_LE AES_128)) (iv:(quad32))  =
  va_pre va_b0 va_s0 stack_b plain_b mask_b cipher_b offset num_blocks key iv /\
   ((va_ensure_total va_b0 va_s0 va_sM va_fM) /\ (va_get_ok va_sM)
    ...
    va_s0) (va_get_mem va_sM)) /\ (let mask = (buffer128_read mask_b 0 (va_get_mem va_s0)) in let
    plain = (buffer128_as_seq (va_get_mem va_sM) plain_b) in let old_cipher = (buffer128_as_seq
    (va_get_mem va_s0) cipher_b) in let cipher = (buffer128_as_seq (va_get_mem va_sM) cipher_b) in
