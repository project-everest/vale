...
let implies_pre (h0:HS.mem) (output_b:b8) (input_b:b8) (key:Ghost.erased (aes_key_LE AES_128)) (keys_b:b8)  (stack_b:b8) : Lemma
  ...
  (ensures (
B.length stack_b == 32 /\ live h0 stack_b /\ buf_disjoint_from stack_b [output_b;input_b;keys_b] /\ (  let buffers = stack_b::output_b::input_b::keys_b::[] in
...
let implies_post (va_s0:va_state) (va_sM:va_state) (va_fM:va_fuel) (output_b:b8) (input_b:b8) (key:Ghost.erased (aes_key_LE AES_128)) (keys_b:b8)  (stack_b:b8) : Lemma
...
let aes128_encrypt_block_be_win output_b input_b key keys_b  =
  ...
  let h0 = get() in
  assert (buf_disjoint_from stack_b [output_b; input_b; keys_b]);
  assert (pre_cond h0 output_b input_b key keys_b);
  assert (B.length stack_b == 32);
  assert (live h0 stack_b);
  st_put h0 (fun h -> pre_cond h output_b input_b key keys_b /\ B.length stack_b == 32 /\ live h stack_b /\ buf_disjoint_from stack_b [output_b;input_b;keys_b]) (ghost_aes128_encrypt_block_be_win output_b input_b key keys_b stack_b);
  pop_frame()
