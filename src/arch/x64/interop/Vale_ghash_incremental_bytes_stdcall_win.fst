...
(h_b:buffer128) (hash_b:buffer128) (input_b:buffer128) (num_bytes:nat64)  =
  ((va_require_total va_b0 (va_code_ghash_incremental_bytes_stdcall_win ()) va_s0) /\
    (va_get_ok va_s0) /\ (locs_disjoint [(loc_buffer stack_b); (loc_buffer h_b); (loc_buffer
    hash_b); (loc_buffer input_b)]) /\ (buffer_readable (va_get_mem va_s0) stack_b) /\
    (buffer_readable (va_get_mem va_s0) h_b) /\ (buffer_readable (va_get_mem va_s0) hash_b) /\
    (buffer_readable (va_get_mem va_s0) input_b) /\ (buffer_length stack_b) >= 10 /\
    ...
    (va_get_reg R9 va_s0) == num_bytes /\ (validSrcAddrs128 (va_get_mem va_s0) (va_get_reg R8
    va_s0) input_b (bytes_to_quad_size num_bytes)) /\ (num_bytes > 0 ==> (va_get_reg R8 va_s0) + 16
...
(h_b:buffer128) (hash_b:buffer128) (input_b:buffer128) (num_bytes:nat64)  =
  ((va_ensure_total va_b0 va_s0 va_sM va_fM) /\ (va_get_ok va_sM)
    ...
    va_sM) h_b) /\ (buffer_readable (va_get_mem va_sM) hash_b) /\ (buffer_readable (va_get_mem
    va_sM) input_b) /\ (num_bytes == 0 ==> (buffer128_read hash_b 0 (va_get_mem va_sM)) ==
    (buffer128_read hash_b 0 (va_get_mem va_s0))) /\ (let input_bytes = (slice_work_around
    (le_seq_quad32_to_bytes (buffer128_as_seq (va_get_mem va_sM) input_b)) num_bytes) in let
    padded_bytes = (pad_to_128_bits input_bytes) in let input_quads = (le_bytes_to_seq_quad32
    padded_bytes) in let h = (buffer128_read h_b 0 (va_get_mem va_s0)) in let old_io =
    (buffer128_read hash_b 0 (va_get_mem va_s0)) in let io = (buffer128_read hash_b 0 (va_get_mem
    va_sM)) in num_bytes > 0 ==> (l_and ((Seq.length input_quads) > 0) (io == (ghash_incremental h
    old_io input_quads)))) /\ (va_state_eq va_sM (va_update_mem va_sM (va_update_flags va_sM
