open Prims
type 'An bv_t = unit FStar_BitVector.bv_t
let (bv_uext :
  Prims.pos -> Prims.pos -> unit bv_t -> Prims.bool FStar_Seq_Base.seq) =
  fun n  ->
    fun i  ->
      fun a  -> FStar_Seq_Base.append (FStar_Seq_Base.create i false) a
...
let (bv2int : Prims.pos -> unit bv_t -> unit FStar_UInt.uint_t) =
  FStar_UInt.from_vec 
let (bv_zero : Prims.pos -> unit bv_t) =
  fun n  -> int2bv n (Prims.parse_int "0") 
...
let (bvmod : Prims.pos -> unit bv_t -> unit FStar_UInt.uint_t -> unit bv_t) =
  fun n  -> fun a  -> fun b  -> int2bv n (FStar_UInt.mod_ n (bv2int n a) b) 
let (bvmul : Prims.pos -> unit bv_t -> unit FStar_UInt.uint_t -> unit bv_t) =
  fun n  ->
    fun a  -> fun b  -> int2bv n (FStar_UInt.mul_mod n (bv2int n a) b)
