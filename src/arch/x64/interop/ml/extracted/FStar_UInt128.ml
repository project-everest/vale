...
let (__proj__Mkuint128__item__low : uint128 -> FStar_UInt64.t) =
  fun projectee  ->
    ...
    | { low = __fname__low; high = __fname__high;_} -> __fname__low
  
let (__proj__Mkuint128__item__high : uint128 -> FStar_UInt64.t) =
  fun projectee  ->
    match projectee with
    | { low = __fname__low; high = __fname__high;_} -> __fname__high
  
type t = uint128
let (v : t -> unit FStar_UInt.uint_t) =
  fun x  ->
    (FStar_UInt64.v x.low) +
      ((FStar_UInt64.v x.high) * (Prims.pow2 (Prims.parse_int "64")))
...
let (uint_to_t : unit FStar_UInt.uint_t -> t) =
  fun x  ->
    {
      low =
        (FStar_UInt64.uint_to_t (x mod (Prims.pow2 (Prims.parse_int "64"))));
      high =
        (FStar_UInt64.uint_to_t (x / (Prims.pow2 (Prims.parse_int "64"))))
    }
  ...
  fun a  ->
    fun b  ->
      FStar_UInt64.shift_right
        ...
        (FStar_UInt32.uint_to_t (Prims.parse_int "63"))
...
let (append_uint :
  ...
  =
  fun n1  ->
    fun n2  -> fun num1  -> fun num2  -> num1 + (num2 * (Prims.pow2 n1))
  

let (vec128 : t -> unit FStar_BitVector.bv_t) =
  fun a  -> FStar_UInt.to_vec (Prims.parse_int "128") (v a) 
let (vec64 : FStar_UInt64.t -> unit FStar_BitVector.bv_t) =
  fun a  -> FStar_UInt.to_vec (Prims.parse_int "64") (FStar_UInt64.v a) 
...
let (shift_left_small : t -> FStar_UInt32.t -> t) =
  fun a  ->
    fun s  ->
      if FStar_UInt32.eq s (FStar_UInt32.uint_to_t (Prims.parse_int "0"))
      then a
      else
        {
...
let (shift_left_large : t -> FStar_UInt32.t -> t) =
  fun a  ->
    fun s  ->
      {
        low = (FStar_UInt64.uint_to_t (Prims.parse_int "0"));
    ...
    fun s  ->
      {
        ...
        high = (FStar_UInt64.uint_to_t (Prims.parse_int "0"))
      }
...
let (uint64_to_uint128 : FStar_UInt64.t -> t) =
  fun a  ->
    { low = a; high = (FStar_UInt64.uint_to_t (Prims.parse_int "0")) }
...
let (u32_combine : FStar_UInt64.t -> FStar_UInt64.t -> FStar_UInt64.t) =
  fun hi  ->
    fun lo  -> FStar_UInt64.add lo (FStar_UInt64.shift_left hi u32_32)
...
let (l32 : unit FStar_UInt.uint_t -> unit FStar_UInt.uint_t) =
  fun x  -> x mod (Prims.pow2 (Prims.parse_int "32")) 
let (h32 : unit FStar_UInt.uint_t -> unit FStar_UInt.uint_t) =
  fun x  -> x / (Prims.pow2 (Prims.parse_int "32")) 
let (mul32_bound :
...
let (mul_wide_low : FStar_UInt64.t -> FStar_UInt64.t -> Prims.int) =
  fun x  ->
    fun y  ->
      ((((plh x y) +
           (((phl x y) + (pll_h x y)) mod (Prims.pow2 (Prims.parse_int "32"))))
          * (Prims.pow2 (Prims.parse_int "32")))
         mod (Prims.pow2 (Prims.parse_int "64")))
        + (pll_l x y)
...
let (mul_wide_high : FStar_UInt64.t -> FStar_UInt64.t -> Prims.int) =
  fun x  ->
    fun y  ->
      ((phh x y) +
         (((phl x y) + (pll_h x y)) / (Prims.pow2 (Prims.parse_int "32"))))
        +
        (((plh x y) +
            (((phl x y) + (pll_h x y)) mod
               (Prims.pow2 (Prims.parse_int "32"))))
           / (Prims.pow2 (Prims.parse_int "32")))
...
let (mul_wide_impl : FStar_UInt64.t -> FStar_UInt64.t -> t) =
  fun x  ->
    fun y  ->
      match mul_wide_impl_t' x y with
      | (u1,w3,x',t') ->
          {
            low =
              (u32_combine'
                 (FStar_UInt64.add
                    (FStar_UInt64.mul u1 (FStar_UInt64.shift_right y u32_32))
                    (u64_mod_32 t')) w3);
            high =
              (FStar_UInt64.add_mod
                 (FStar_UInt64.add
                    (FStar_UInt64.mul x' (FStar_UInt64.shift_right y u32_32))
                    (FStar_UInt64.shift_right t' u32_32))
                 (FStar_UInt64.shift_right
                    (FStar_UInt64.add
                       (FStar_UInt64.mul u1
