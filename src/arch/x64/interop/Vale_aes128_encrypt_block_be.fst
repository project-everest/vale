module Vale_aes128_encrypt_block_be
...
val va_code_aes128_encrypt_block_be: unit -> va_code
let va_code_aes128_encrypt_block_be () = va_code_AESEncryptBlockStdcall_BE is_win AES_128
...
((va_require_total va_b0 (va_code_AESEncryptBlockStdcall_BE win alg) va_s0) /\
    ...
    (locs_disjoint [(loc_buffer input_buffer); (loc_buffer output_buffer); (loc_buffer
    keys_buffer)]) /\ (alg = AES_128 || alg = AES_256) /\ (buffer128_read input_buffer 0
    (va_get_mem va_s0)) == input /\ expanded_key_ptr == (buffer_addr keys_buffer (va_get_mem
    va_s0)) /\ (validSrcAddrs128 (va_get_mem va_s0) input_ptr input_buffer 1) /\ (validSrcAddrs128
    (va_get_mem va_s0) output_ptr output_buffer 1) /\ (validSrcAddrs128 (va_get_mem va_s0)
    expanded_key_ptr keys_buffer ((nr alg) + 1)) /\ (forall (i:int) . (0 <= i && i < (nr alg) + 1)
...
val va_lemma_aes128_encrypt_block_be(va_b0:va_code) (va_s0:va_state)
...
let va_lemma_aes128_encrypt_block_be va_b0 va_s0 output_b input_b key keys_b =
