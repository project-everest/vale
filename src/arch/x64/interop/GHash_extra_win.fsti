...
let pre_cond (h:HS.mem) (in_b:b8) (hash_b:b8) (h_b:b8) (num_bytes:nat64) (orig_hash:Ghost.erased (quad32)) = live h in_b /\ live h hash_b /\ live h h_b /\ bufs_disjoint [in_b;hash_b;h_b] /\ length in_b % 16 == 0 /\ length hash_b % 16 == 0 /\ length h_b % 16 == 0 /\
...
let full_post_cond (h:HS.mem) (h':HS.mem) (in_b:b8) (hash_b:b8) (h_b:b8) (num_bytes:nat64) (orig_hash:Ghost.erased (quad32)) = 
    let mods = M.loc_buffer hash_b in
    M.modifies mods h h' /\ post_cond h h' in_b hash_b h_b num_bytes orig_hash

val ghash_incremental_extra_stdcall_win: in_b:b8 -> hash_b:b8 -> h_b:b8 -> num_bytes:UInt64.t -> orig_hash:Ghost.erased (quad32) -> Stack unit
