...
let pre_cond (h:HS.mem) (h_b:b8) (hash_b:b8) (input_b:b8) (num_bytes:nat64) = live h h_b /\ live h hash_b /\ live h input_b /\ bufs_disjoint [h_b;hash_b;input_b] /\ length h_b % 16 == 0 /\ length hash_b % 16 == 0 /\ length input_b % 16 == 0 /\     B.length input_b == 16 * (bytes_to_quad_size num_bytes) /\ B.length h_b >= 16 /\ B.length hash_b >= 16
...
let full_post_cond (h0:HS.mem) (h1:HS.mem) (h_b:b8) (hash_b:b8) (input_b:b8) (num_bytes:nat64) =
    M.modifies (M.loc_buffer hash_b) h0 h1 /\
    post_cond h0 h1 h_b hash_b input_b num_bytes

val ghash_incremental_bytes_stdcall_win: h_b:b8 -> hash_b:b8 -> input_b:b8 -> num_bytes:UInt64.t -> Stack unit
