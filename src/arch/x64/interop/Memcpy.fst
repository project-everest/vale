...
let create_initial_state is_win dst src stack_b (h0:HS.mem{pre_cond h0 dst src /\ B.length stack_b == 24 /\ live h0 stack_b /\ buf_disjoint_from stack_b [dst;src]}) : GTot TS.traceState =
  ...
  let (s_b:BS.state) = {BS.ok = true; BS.regs = regs; BS.xmms = xmms; BS.flags = 0; BS.mem = Interop.down_mem h0 addrs buffers} in
...
let create_lemma is_win dst src stack_b (h0:HS.mem{pre_cond h0 dst src /\ B.length stack_b == 24 /\ live h0 stack_b /\ buf_disjoint_from stack_b [dst;src]}) : Lemma
  (state_of_S (create_initial_state is_win dst src stack_b h0) == create_initial_vale_state is_win dst src stack_b h0) =
    let s_init = create_initial_state is_win dst src stack_b h0 in
...
val lemma_ghost_memcpy:  is_win:bool -> dst:s8 -> src:s8 ->  stack_b:b8 -> (h0:HS.mem{pre_cond h0 dst src /\ B.length stack_b == 24 /\ live h0 stack_b /\ buf_disjoint_from stack_b [dst;src]}) ->
  Ghost (TS.traceState * nat * HS.mem)
    ...
    (ensures (fun (s1, f1, h1) ->
      (let s0 = create_initial_state is_win dst src stack_b h0 in
