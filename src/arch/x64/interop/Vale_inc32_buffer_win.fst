...
(iv_b:buffer128)  =
   ((va_require_total va_b0 (va_code_inc32_buffer_win ()) va_s0) /\ (va_get_ok va_s0) /\
    (locs_disjoint [(loc_buffer stack_b); (loc_buffer iv_b)]) /\ (buffer_readable (va_get_mem
    va_s0) stack_b) /\ (buffer_readable (va_get_mem va_s0) iv_b) /\ (buffer_length iv_b) == 1 /\
    (buffer_length stack_b) >= 2 /\ (valid_stack_slots (va_get_mem va_s0) (va_get_reg Rsp va_s0)
    stack_b 0) /\ (va_get_reg Rcx va_s0) == (buffer_addr iv_b (va_get_mem va_s0)))
...
(iv_b:buffer128)  =
  ((va_ensure_total va_b0 va_s0 va_sM va_fM) /\ (va_get_ok va_sM)
    ...
    (va_get_xmm 15 va_sM) == (va_get_xmm 15 va_s0) /\ (modifies_buffer128 iv_b (va_get_mem va_s0)
    (va_get_mem va_sM)) /\ (buffer128_read iv_b 0 (va_get_mem va_sM)) == (inc32 (buffer128_read
    iv_b 0 (va_get_mem va_s0)) 1) /\ (va_state_eq va_sM (va_update_mem va_sM (va_update_flags va_sM
