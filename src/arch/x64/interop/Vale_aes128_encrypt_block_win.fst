module Vale_aes128_encrypt_block_win
...
val va_code_aes128_encrypt_block_win: unit -> va_code
let va_code_aes128_encrypt_block_win () = va_code_AESEncryptBlockStdcall is_win AES_128
...
let vale_pre (va_b0:va_code) (va_s0:va_state) (win:bool) (alg:algorithm)
  ...
  (keys_buffer:buffer128) =
    ((va_require_total va_b0 (va_code_AESEncryptBlockStdcall win alg) va_s0) /\ (va_get_ok
    va_s0) /\ (let output_ptr = (if win then (va_get_reg rcx va_s0) else (va_get_reg rdi va_s0)) in
    let input_ptr = (if win then (va_get_reg rdx va_s0) else (va_get_reg rsi va_s0)) in let
    expanded_key_ptr = (if win then (va_get_reg r8 va_s0) else (va_get_reg rdx va_s0)) in (alg =
    AES_128 || alg = AES_256) /\ (buffer128_read input_buffer 0 (va_get_mem va_s0)) == input /\
    expanded_key_ptr == (buffer_addr keys_buffer (va_get_mem va_s0)) /\ (validSrcAddrs128
    (va_get_mem va_s0) input_ptr input_buffer 1 (va_get_memTaint va_s0) Secret) /\
    (validSrcAddrs128 (va_get_mem va_s0) output_ptr output_buffer 1 (va_get_memTaint va_s0) Secret)
    /\ (validSrcAddrs128 (va_get_mem va_s0) expanded_key_ptr keys_buffer ((nr alg) + 1)
    (va_get_memTaint va_s0) Secret) /\ (forall (i:int) . (0 <= i && i < (nr alg) + 1) ==>
    (buffer128_read keys_buffer i (va_get_mem va_s0)) == (index (key_to_round_keys_LE alg key)
    i))))
...
let vale_post (va_b0:va_code) (va_s0:va_state) (win:bool) (alg:algorithm)
  ...
  ((va_ensure_total va_b0 va_s0 va_sM va_fM) /\ (va_get_ok va_sM)
    ...
    /\ (buffer128_read output_buffer 0 (va_get_mem va_sM)) == (aes_encrypt_LE alg key input)) /\
    (va_state_eq va_sM (va_update_trace va_sM (va_update_flags va_sM (va_update_xmm 2 va_sM
    (va_update_xmm 0 va_sM (va_update_mem va_sM (va_update_reg r8 va_sM (va_update_ok va_sM
    va_s0)))))))))
...
val va_lemma_aes128_encrypt_block_win (va_b0:va_code) (va_s0:va_state) (stack_b:buffer64)
...
let va_lemma_aes128_encrypt_block_win va_b0 va_s0 stack_b output_b input_b key keys_b =
  let input = buffer128_read input_b 0 va_s0.mem in
  va_lemma_AESEncryptBlockStdcall va_b0 va_s0 is_win AES_128 input key input_b output_b keys_b
