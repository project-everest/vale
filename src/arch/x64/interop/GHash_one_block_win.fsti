...
let pre_cond (h:HS.mem) (h_b:b8) (hash_b:b8) (input_b:b8) (offset:nat64) = live h h_b /\ live h hash_b /\ live h input_b /\ bufs_disjoint [h_b;hash_b;input_b] /\ length h_b % 16 == 0 /\ length hash_b % 16 == 0 /\ length input_b % 16 == 0 /\
  B.length h_b >= 16 /\ B.length hash_b >= 16 /\ B.length input_b >= 16 * (offset + 1)

let partial_post_cond (h:HS.mem) (h':HS.mem) (h_b:b8) (hash_b:b8) (input_b:b8) (offset:nat64) = length h_b % 16 == 0 /\ length hash_b % 16 == 0 /\ length input_b % 16 == 0 /\
 B.length h_b >= 16 /\ B.length hash_b >= 16 /\ B.length input_b >= 16 * (offset + 1) /\
...
let post_cond (h:HS.mem) (h':HS.mem) (h_b:b8) (hash_b:b8) (input_b:b8) (offset:nat64) =
    M.modifies (M.loc_buffer hash_b) h h' /\
    partial_post_cond h h' h_b hash_b input_b offset


val ghash_incremental_one_block_buffer_win: h_b:b8 -> hash_b:b8 -> input_b:b8 -> offset:UInt64.t -> Stack unit
