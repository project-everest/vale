...
B.length stack_b == 40 /\ live h0 stack_b /\ buf_disjoint_from stack_b [plain_b;mask_b;cipher_b] /\ (  let taint_func (x:b8) : GTot taint =
  ...
  let regs = Map16_i.upd regs rcx addr_plain_b in
  let regs = Map16_i.upd regs rdx addr_mask_b in
  let regs = Map16_i.upd regs r8 addr_cipher_b in  
  let regs = Map16_i.upd regs r9 offset in  
