...
let implies_pre (h0:HS.mem) (plain_b:b8) (mask_b:b8) (cipher_b:b8) (offset:nat64) (num_blocks:Ghost.erased (nat64)) (key:Ghost.erased (aes_key_LE AES_128)) (iv:Ghost.erased (quad32))  (stack_b:b8) : Lemma
  (requires pre_cond h0 plain_b mask_b cipher_b offset num_blocks key iv /\ B.length stack_b == 40 /\ live h0 stack_b /\ bufs_disjoint [stack_b;plain_b;mask_b;cipher_b])
  (ensures (
B.length stack_b == 40 /\ live h0 stack_b /\ bufs_disjoint [stack_b;plain_b;mask_b;cipher_b] /\ (  let buffers = stack_b::plain_b::mask_b::cipher_b::[] in
...
let implies_post (va_s0:va_state) (va_sM:va_state) (va_fM:va_fuel) (plain_b:b8) (mask_b:b8) (cipher_b:b8) (offset:nat64) (num_blocks:Ghost.erased (nat64)) (key:Ghost.erased (aes_key_LE AES_128)) (iv:Ghost.erased (quad32))  (stack_b:b8) : Lemma
  (requires pre_cond va_s0.mem.hs plain_b mask_b cipher_b offset num_blocks key iv /\ B.length stack_b == 40 /\ live va_s0.mem.hs stack_b /\ bufs_disjoint [stack_b;plain_b;mask_b;cipher_b]/\
...
val ghost_gcm_load_xor_store_buffer_win: plain_b:b8 -> mask_b:b8 -> cipher_b:b8 -> offset:nat64 -> num_blocks:Ghost.erased (nat64) -> key:Ghost.erased (aes_key_LE AES_128) -> iv:Ghost.erased (quad32) ->  stack_b:b8 -> (h0:HS.mem{pre_cond h0 plain_b mask_b cipher_b offset num_blocks key iv /\ B.length stack_b == 40 /\ live h0 stack_b /\ bufs_disjoint [stack_b;plain_b;mask_b;cipher_b]}) -> GTot (h1:HS.mem{post_cond h0 h1 plain_b mask_b cipher_b offset num_blocks key iv })
...
let gcm_load_xor_store_buffer_win plain_b mask_b cipher_b offset num_blocks key iv  =
  ...
  let h0 = get() in
  st_put h0 (fun h -> pre_cond h plain_b mask_b cipher_b (UInt64.v offset) num_blocks key iv /\ B.length stack_b == 40 /\ live h stack_b /\ bufs_disjoint [stack_b;plain_b;mask_b;cipher_b]) (ghost_gcm_load_xor_store_buffer_win plain_b mask_b cipher_b (UInt64.v offset) num_blocks key iv stack_b);
  pop_frame()
