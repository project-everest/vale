...
B.length stack_b == 16 /\ live h0 stack_b /\ buf_disjoint_from stack_b [b] /\ (  let taint_func (x:b8) : GTot taint =
  ...
  let regs = Map16_i.upd regs rcx addr_b in
...
val ghost_zero_quad32_buffer_win: b:s8 ->  stack_b:b8 -> (h0:HS.mem{pre_cond h0 b /\ B.length stack_b == 16 /\ live h0 stack_b /\ buf_disjoint_from stack_b [b]}) -> GTot (h1:HS.mem{post_cond h0 h1 b })

let ghost_zero_quad32_buffer_win b stack_b h0 =
  ...
  // Ensures that va_code_zero_quad32_buffer_win is actually Vale code, and that s1 is the result of executing this code
