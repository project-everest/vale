...
(* Abstract maps linking buffers to addresses in the Vale heap. *)
...
type addr_map = (m:(t:Type -> (B.buffer t) -> nat64){
  forall (buf1 buf2:B.buffer UInt64.t). B.disjoint buf1 buf2 ==> 
    disjoint_addr (m UInt64.t buf1) (B.length buf1) (m UInt64.t buf2) (B.length buf2)})
