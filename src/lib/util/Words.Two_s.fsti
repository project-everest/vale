...
unfold
let nat_to_two_unfold (size:nat) (n:natN (pow2_norm (2 * size))) : two (natN (pow2_norm size)) =
  let n1 = pow2_norm size in
  let n2 = pow2_norm (2 * size) in
  Mktwo (n % n1) ((n / n1) % n1)

let nat_to_two (size:nat) (n:natN (pow2_norm (2 * size))) : two (natN (pow2_norm size)) =
...
let two_to_nat (size:nat) (x:two (natN (pow2_norm size))) : natN (pow2_norm (2 * size)) =
