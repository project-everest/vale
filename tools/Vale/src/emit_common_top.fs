...
let build_decls (env:env) (includes: (string * bool * (((loc * decl) * bool) list)) list) (ds:((loc * decl) * bool) list):decls =
  let ds =
  ...
  let ds = if !do_typecheck then tc_decls includes ds else ds in
