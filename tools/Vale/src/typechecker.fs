...
let rec unify_one env (m:substitutions) (s:typ) (t:typ):substitutions =
  ...
  | (TApp (x, ys), TApp (u, vs)) ->
    if x = u then
      ...
      if t1 = t2 then m else err ("cannot unify type \"" + string_of_typ t1 + "\" and \"" + string_of_typ t2 + "\"")
  ...
  | (TName x, TName y) ->
    if x = y then m
    else err ("cannot unify type \"" + string_of_typ t1 + "\" and \"" + string_of_typ t2 + "\"")
  | _ -> err ("cannot unify type \"" + string_of_typ t1 + "\" and \"" + string_of_typ t2 + "\"")
...
let tc_exp (env:env) (e:exp) (et:typ option):(typ list * exp) =
  try
    ...
    printfn "ts = %A  ae = %A  cl = %A  et = %A  e = %A" ts ae cl et e;
