...
// wrap "precedes" to avoid issues with label (precedes ...)
let precedes_wrap (#a:Type) (x:a) (y:a) : GTot Type0 = precedes x y
...
let wp_If (#a:Type) (#c1:code) (#c2:code) (b:cmp) (qc1:quickCode a c1) (qc2:quickCode a c2) (mods:mods_t) (s0:va_state) (k:va_state -> a -> Type0) : Type0 =
  ...
  (forall (f:nat64). let s1 = va_upd_flags f s0 in
...
let wp_While_body
    ...
    : Type0 =
  valid_cmp b s1 /\
  (forall (f:nat64). let s1' = va_upd_flags f s1 in
...
let va_state_match (s0:va_state) (s1:va_state) : Pure Type0
  ...
  =
  assert_norm (all_regs_match s0.regs s1.regs ==> Regs.equal s0.regs s1.regs);
  assert_norm (all_xmms_match s0.xmms s1.xmms ==> Xmms.equal s0.xmms s1.xmms);
  s0.ok == s1.ok /\
